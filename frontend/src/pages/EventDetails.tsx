...
export default function EventDetails() {
	...
	useEffect(() => {
		...
		// 	})
		// 	.catch((err) => {
		const event = events.findIndex((ev) => ev.id === id);
		if (event === -1) {
		...
		setEvent({
			...
			endDate: new Date(events[event].endDate),
			updatedAt: new Date(events[event].updatedAt),
			createdAt: new Date(events[event].createdAt),
			users: (events[event].users ?? []).map((u: any) => ({
				...u,
		...
		// 	if (event) setEvent(event as unknown as Event);
